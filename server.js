...
const app = express();
app.set('trust proxy', 1); // Proxy arkas?nda (Render/Railway) çal?ºt??? için gerekli
...
passport.use(new GoogleStrategy({
  ...
  async (req, accessToken, refreshToken, profile, done) => {
    try {
      ...
      // 2. Email ile ara (Hesap eºleºtirme)
...
const BADGES = {
  ...
  STREAK_7: { id: 'streak_7', icon: '?', name: 'Haftal?k Seri', desc: '7 gün üst üste çal?ºt?n!' },
  STREAK_30: { id: 'streak_30', icon: '??', name: 'Ayl?k Seri', desc: '30 gün üst üste çal?ºt?n! ?nan?lmaz!' },
  KNOWN_100: { id: 'known_100', icon: '??', name: 'Kelime Avc?s?', desc: '100 kelime ö?rendin!' },
  KNOWN_500: { id: 'known_500', icon: '??', name: 'Kelime Ustas?', desc: '500 kelime ö?rendin!' },
  KNOWN_1000: { id: 'known_1000', icon: '??', name: 'Kelime Kral?', desc: '1000 kelime ö?rendin!' },
  NIGHT_OWL: { id: 'night_owl', icon: '??', name: 'Gece Kuºu', desc: 'Gece 00:00 - 05:00 aras? çal?ºt?n.' },
  EARLY_BIRD: { id: 'early_bird', icon: '??', name: 'Erkenci Kuº', desc: 'Sabah 05:00 - 09:00 aras? çal?ºt?n.' },
  WEEKEND_WARRIOR: { id: 'weekend_warrior', icon: '??', name: 'Hafta Sonu Savaºç?s?', desc: 'Hafta sonu çal?ºmay? ihmal etmedin.' }
};
    ...
    try {
      const deleted = await User.deleteMany({
        $or: [
          ...
          { username: null },
          { username: "" },
          { "username": { $type: "string", $regex: /^\s*$/ } }, // sadece boºluk içerenler
          { isVerified: false } // Do?rulanmam?º hesaplar? sil
        ]
      });
...
async function sendVerificationEmail(email, username, code) {
  ...
  const mailPromise = transporter.sendMail({
      ...
      text: `Merhaba ${username},\n\nHesab?n? do?rulamak için kodun: ${code}\n\n?yi çal?ºmalar!`,
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
          <h2 style="color: #FF9F1C;">WordBoost Do?rulama</h2>
          <p>Merhaba <strong>${username}</strong>,</p>
          <p>Hesab?n? do?rulamak için aºa??daki kodu kullanabilirsin:</p>
          ...
          <p style="font-size: 12px; color: #999;">E?er bu iºlemi sen yapmad?ysan, bu maili görmezden gelebilirsin.</p>
        </div>
      `
  });

  const timeoutPromise = new Promise((_, reject) =>
    setTimeout(() => reject(new Error('Mail gönderimi zaman aº?m?na u?rad? (SMTP yan?t vermiyor)')), MAIL_SEND_TIMEOUT_MS)
  );
  ...
  try {
    ...
    // Hata detay?n? logla ve üst seviyeye anlaml? bir mesaj döndür
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    if (!emailRegex.test(email)) {
      return res.status(400).json({ error: "Geçersiz email format?" });
    }
    ...
    if (user) {
      // E?er kullan?c? var ama do?rulanmam?ºsa, kayd? güncelle ve tekrar mail at
      if (!user.isVerified) {
        // HESAP KURTARMA / ÜZER?NE YAZMA (Unverified accounts only)
        // E?er kullan?c? ad? veya email eºleºiyorsa ve hesap do?rulanmam?ºsa,
        // yeni gelen kiºi bu hesab? devralabilir (email ve ºifresini güncelleyerek).
    ...
    if (!mailResult.success) {
      ...
      // Hesab? oluºturduk ama mail gidemedi -> Kullan?c?ya aç?kça söyle
      return res.status(500).json({
...
app.post('/api/forgot-password', async (req, res) => {
  try {
    ...
    if (!user) {
      // Güvenlik: Kullan?c? yoksa bile "gönderildi" de (User enumeration prevention)
      // Ama user experience için ºimdilik hata dönelim
    ...
    res.json({ success: true, message: "S?f?rlama kodu gönderildi" });
...
app.post('/api/reset-password', async (req, res) => {
  try {
    ...
    if (user.verificationCode !== code || user.verificationCodeExpires < Date.now()) {
      return res.status(400).json({ error: "Geçersiz veya süresi dolmuº kod" });
    }
    ...
    res.json({ success: true, message: "ªifre baºar?yla güncellendi" });
...
app.post('/api/verify-email', async (req, res) => {
  try {
    ...
    if (user.isVerified) return res.status(400).json({ error: "Hesap zaten do?rulanm?º" });
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (!valid) {
      return res.status(400).json({ error: "ªifre yanl?º" });
    }

    // Do?rulama kontrolü (Opsiyonel: E?er zorunluysa buray? aç)
    /*
    if (!user.isVerified) {
      return res.json({ 
        success: false, 
        requireVerification: true, 
        email: user.email,
        error: "Lütfen önce mail adresinizi do?rulay?n" 
      });
    }
    */
...
app.post('/api/profile/update', async (req, res) => {
  try {
    ...
    // Username de?iºimi ve unique kontrolü
    if (username && username !== user.username) {
      // Format kontrolü (boºluk olmamal?, min 3 karakter)
      if (username.length < 3 || /\s/.test(username)) {
        return res.status(400).json({ error: "Kullan?c? ad? en az 3 karakter olmal? ve boºluk içermemelidir." });
      }

      const existing = await User.findOne({ username });
      if (existing) {
        return res.status(400).json({ error: "Bu kullan?c? ad? zaten al?nm?º." });
      }
      user.username = username;
    }
...
app.get('/api/leaderboard', async (req, res) => {
  try {
    // STREAK'e göre s?rala (Önce en yüksek seri, sonra en çok bilinen kelime)
    const users = await User.find({
      username: { $exists: true, $ne: "" },
      "stats.known": { $exists: true },
      isVerified: true // Sadece do?rulanm?º kullan?c?lar
    })
      .sort({ "streak": -1, "stats.known": -1 }) // Önce seri, sonra puan
      .limit(50)
      .select("username nickname avatar stats badges streak");

    // Boº kullan?c?lar? filtrele (ek güvenlik)
...
app.post('/api/stats/update', async (req, res) => {
  try {
    ...
    if (!lastStudy) {
      ...
      // Bugün zaten çal?ºm?º, streak de?iºmez
      user.lastStudyDate = new Date();
    } else if (today.getTime() === lastStudy.getTime() + 86400000) {
      // Dün çal?ºm?º, streak artar
      user.streak += 1;
      user.lastStudyDate = new Date();
    } else {
      // Dünden önce çal?ºm?º, streak s?f?rlan?r (veya 1 olur)
      user.streak = 1;
      user.lastStudyDate = new Date();
    }
...
app.post('/api/rooms', async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: "Room oluºturulamad?" });
  }
});
...
// Yard?mc? fonksiyon: Oda kullan?c?lar?n? stats'tan oluºtur
...
io.on('connection', (socket) => {
  ...
  // ODA OLUªTURMA - Host burada belirlenir!
  socket.on('create-room', async ({ username, avatar }, callback) => {
  try {

    if (!username || username.trim().length < 2) {
      callback?.({ success: false, error: 'Geçerli kullan?c? ad? girin' });
      return;
    }
...
io.to(roomCode).emit('sync-stats', {
  ...
  try {
    ...
    if (!username || username.trim().length < 2) {
      if (callback) callback({ success: false, error: 'Geçerli kullan?c? ad? girin' });
      return;
    }
    ...
    if (!room || !room.isActive) {
      ...
      // Ayn? kullan?c? ad? kontrolü (odada aktif olanlar aras?nda)
      ...
      // Host mu kontrol et (server taraf?nda güvenlik!)
      ...
      // Odadaki tüm kullan?c?lar? topla (güncel stats ile)
      const users = getUsersFromStats(roomCode);
  ...
  // KEL?ME DE??ªT?RME (sadece host)
  socket.on('change-word', ({ roomCode, wordIndex }) => {
    try {
      const user = roomUsers.get(socket.id);
      const hostName = roomHosts.get(roomCode);
      
      // Güvenlik kontrolü: Sadece gerçek host de?iºtirebilir
