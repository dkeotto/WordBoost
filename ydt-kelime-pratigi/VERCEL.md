...
- **AI Sohbet (çoklu thread):** Backend’de `GET/POST /api/ai/chat/threads`, `PATCH/DELETE /api/ai/chat/threads/:id`, `GET /api/ai/chat?threadId=`, `POST /api/ai/chat/stream` (gövde: `message`, `threadId`, isteğe bağlı `attachments: [{ name, mimeType, text }]` — UTF-8 metin dosyaları). İlk deploy sonrası MongoDB’de eski `userId` **unique** indeksi kalkmazsa `AiChatThread.syncIndexes()` uyarı verebilir; gerekirse koleksiyonda `userId` üzerindeki unique indeksi elle kaldır.
- **Railway (backend):** **Groq:** `GROQ_API_KEY` (ve isteğe bağlı `GROQ_MODEL`, varsayılan `llama-3.3-70b-versatile`). `AI_PROVIDER=groq` ile zorunlu Groq; boş bırakıp yalnızca `GROQ_API_KEY` doldurursan varsayılan sağlayıcı Groq olur. **Vercel AI Gateway yedek:** `AI_GATEWAY_API_KEY` (veya `VERCEL_AI_GATEWAY_API_KEY`) + isteğe bağlı `AI_GATEWAY_MODEL` (varsayılan `openai/gpt-4o-mini`), `AI_GATEWAY_BASE_URL` (varsayılan `https://ai-gateway.vercel.sh/v1`). `GROQ_API_KEY` ile birlikte tanımlıysa sunucu **otomatik failover** kullanır (429 / hız limitinde diğer uca geçer). `AI_FAILOVER=0` veya `false` ile yalnız Groq. **Anthropic:** `AI_PROVIDER=anthropic` veya Groq anahtarı yokken `ANTHROPIC_API_KEY` + tarihli `ANTHROPIC_MODEL` (örn. `claude-3-5-sonnet-20241022`). [Groq modeller](https://console.groq.com/docs/models), [Anthropic modeller](https://docs.anthropic.com/en/docs/about-claude/models).
